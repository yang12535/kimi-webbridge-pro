...
Bash 中也有类似问题，尤其是中文和嵌套 JSON 同时出现时。`invoke.sh` 不依赖 `jq`，
并支持从 UTF-8 JSON 文件读取动作参数。文件输入不是为了改变协议，而是把 shell
转义和字符编码从请求逻辑中分离出去。
...
`screenshot.py` 和 `screenshot.ps1` 同时兼容两种形式，并统一给 agent 一个本地
文件路径。这也是 `SKILL.md` 不建议直接调用截图动作的原因。
...
Python helper 共用 `webbridge_client.py`，因此 UTF-8 输出、HTTP 错误和 daemon 错误
...
批量图片提取或下载没有作为默认 helper 提供。它会扩大页面抓取范围，容易把头像、
追踪像素、广告和无关私人图片一并保存，也可能带来版权和存储问题。确有需要时，应先
限定正文区域、文件数量和保留目的，再执行针对性的提取。

通用文章正文提取也没有内置站点选择器表。站点 DOM 变化快，预设选择器容易在不报错
的情况下抓到评论、推荐或隐藏内容。需要正文时，优先使用 snapshot 的完整落盘模式，
再让 Agent 只读取与任务相关的部分。
...
`kimi-webbridge-pro` 独立存在，承担跨 Agent 场景下的增强：

- Windows 原生 helper
- Bash/POSIX 调用 helper
- 大型 snapshot 的客户端精简与落盘
- 更严格的标签页所有权
- 新旧截图协议兼容
- 明确的触发边界和失败恢复
